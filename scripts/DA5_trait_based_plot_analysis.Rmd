...
traits_wide_ts <- traits_ts %>% ungroup() %>% pivot_wider(names_from = c(trait),
                                      values_from = c(value)) %>% view()
...
# Site by elevation
traits_ts$site <- factor(traits_ts$site, 
                    levels = c("ACJ", "TRE", "QUE"))
...
(traits_ts %>%
  ...
  ggplot() +
geom_boxplot(aes(x=site, y=log(value),
                    color = treatment),
                    width=0.2,
...
# # No elevation values here, so check original dataset
 elev_data <- traits_wide %>%  ungroup() %>% select(site, treatment, plot_id, elevation) %>% unique()
# 
...
# Change plot_ID to concatenate site, treatment and plot number
 traits_ts <- traits_ts %>%  
   mutate(plot_id = paste0(site,"_" ,treatment, "_",plot_id))
 traits_ts <- traits_ts %>% left_join(elev_data, by = c("site", "treatment", "plot_id"))

         
# Pivot wider
traits_ts_wide <- traits_ts  %>% 
  pivot_wider(names_from = c(trait), values_from = c(value))
```

# Predicted mean plots for the raw Bootstrapped data

```{r}

analysis_site_bs_raw  = vector(mode = "list", length = nrow(traits_ts %>% distinct(trait)))
analysis_site_bs_predict  = traits_ts %>%
  #split by site & treatment
  split(list(.$trait))

trait_names = traits_ts %>% distinct(trait)

for (i in 1:length(analysis_site_bs_raw)) {
  
  analysis_site_bs_raw[[i]] <- lmer(log(value) ~ treatment*site + (1|elevation), data = analysis_site_bs_predict[[i]])
  
  # Use 'predict' on models
  analysis_site_bs_predict[[i]]$predicted_SLA <- 
    predict(analysis_site_bs_raw[[i]], newdata = analysis_site_bs_predict[[i]], re.form = NULL, allow.new.levels = TRUE) 
...
SLA_analysis_site_CWM <- lmer(log(value) ~ treatment*site + (1|elevation), data = traits_ts %>% filter(trait == "sla_cm2_g"))
anova(SLA_analysis_site_CWM)
summary(SLA_analysis_site_CWM)

# Create newdate for CWM SLA
newdata_CWM_SLA <- traits_ts %>% filter(trait == "sla_cm2_g")
...
LA_analysis_CWM_site <- lmer(log(leaf_area_cm2) ~ treatment*site + (1|elevation), data = traits_ts_wide %>% ungroup() %>% select(site, treatment, elevation, leaf_area_cm2) %>% na.omit())
anova(LA_analysis_CWM_site)
summary(LA_analysis_CWM_site)

# Create newdate for CWM SLA
newdata_CWM_LA <- traits_ts_wide %>% ungroup() %>% select(treatment, site,  elevation, leaf_area_cm2) %>% na.omit()
# Use 'predict' on models
