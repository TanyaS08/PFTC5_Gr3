...
SLA_analysis_site_CWM <- lmer(log(sla_cm2_g) ~ treatment*site + (1|elevation.x), data = traits_ts_wide %>% ungroup() %>% select(site, treatment, elevation.x, sla_cm2_g) %>% na.omit())
anova(SLA_analysis_site_CWM)
summary(SLA_analysis_site_CWM)

# Create newdate for CWM SLA
newdata_CWM_SLA <- traits_ts_wide %>% ungroup() %>% select(site, treatment, elevation.x, sla_cm2_g) %>% na.omit()
...
LA_analysis_CWM_site <- lmer(log(leaf_area_cm2) ~ treatment*site + (1|elevation.x), data = traits_ts_wide %>% ungroup() %>% select(site, treatment, elevation.x, leaf_area_cm2) %>% na.omit())
anova(LA_analysis_CWM_site)
summary(LA_analysis_CWM_site)

# Create newdate for CWM SLA
newdata_CWM_LA <- traits_ts_wide %>% ungroup() %>% select(treatment, site,  elevation.x, leaf_area_cm2) %>% na.omit()
# Use 'predict' on models
