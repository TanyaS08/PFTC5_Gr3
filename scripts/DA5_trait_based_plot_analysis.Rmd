...
But also note, that elevation isn't equivilant for different treatments. Site is used here instead of elevation above.

Same thing for the community weighted mean ones below, though potentially less issues with random effects as values are already averaged. 
...
SLA_analysis_site <- lmer(log(sla_cm2_g) ~ treatment*site + (1|plot_id) + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(SLA_analysis_site)
summary(SLA_analysis_site)

# create new dataset with only relevant factors from the model
newdata_SLA <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, taxon, functional_group, season, sla_cm2_g)
...
# Leaf Area
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
LA_analysis_site <- lmer(log(leaf_area_cm2) ~ treatment*site + (1|plot_id) + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(LA_analysis_site)
summary(LA_analysis_site)

# create new dataset with only relevant factors from the model
newdata_LA <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, taxon, functional_group, season, leaf_area_cm2)
...
# Plant Height
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
PH_analysis_site <- lmer(log(plant_height_cm) ~ treatment*site + (1|plot_id) + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(PH_analysis_site)
summary(PH_analysis_site)

# create new dataset with only relevant factors from the model
newdata_PH <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, taxon, functional_group, season, plant_height_cm)
...
# Leaf Thickness
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
LT_analysis_site <- lmer(log(leaf_thickness_mm) ~ treatment*site + (1|plot_id) + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(LT_analysis_site)
summary(LT_analysis_site)

# create new dataset with only relevant factors from the model
newdata_LT <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, taxon, functional_group, season, leaf_thickness_mm)
...
# LDMC
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
LDMC_analysis_site <- lmer(log(ldmc) ~ treatment*site + (1|plot_id) + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(LDMC_analysis_site)
summary(LDMC_analysis_site)

# create new dataset with only relevant factors from the model
newdata_LDMC <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, taxon, functional_group, season, ldmc)
...
```{r}
SLA_analysis_elev <- lmer(log(sla_cm2_g) ~ treatment*elevation + (1|site/plot_id) + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
...
traits_ts <- read.csv("C:/Users/sge010/Documents/PFTC Projects/PFTC_Fire_Manuscript/PFTC5_Gr3/data/processed/traits_traitstrapped_raw.csv", header = TRUE)
traits_wide_ts <- traits_ts %>% pivot_wider(names_from = c(trait),
                                      values_from = c(value))

# Community weighted means instead
traits_CWM <- read.csv("C:/Users/sge010/Documents/PFTC Projects/PFTC_Fire_Manuscript/PFTC5_Gr3/data/processed/traits_traitstrapped_moments.csv", header = TRUE)
...
# SLA
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
SLA_analysis_site_CWM <- lmer(log(sla_cm2_g) ~ treatment*site + (1|plot_id), data = traits_CWM_wide)
anova(SLA_analysis_site_CWM)
summary(SLA_analysis_site_CWM)

# Create newdate for CWM SLA
newdata_CWM_SLA <- traits_CWM_wide %>% ungroup() %>% select(site, treatment, plot_id, sla_cm2_g)
...
LA_analysis_CWM_site <- lmer(log(leaf_area_cm2) ~ treatment*site + (1|plot_id),
                        data = traits_CWM_wide)
anova(LA_analysis_CWM_site)
summary(LA_analysis_CWM_site)

# Create newdate for CWM SLA
newdata_CWM_LA <- traits_CWM_wide %>% ungroup() %>% select(site, treatment, plot_id, leaf_area_cm2)
...
PH_analysis_CWM_site <- lmer(log(plant_height_cm) ~ treatment*site + (1|plot_id),
                        data = traits_CWM_wide)
anova(PH_analysis_CWM_site)
summary(PH_analysis_CWM_site)

# Create newdate for CWM SLA
newdata_CWM_PH <- traits_CWM_wide %>% ungroup() %>% select(site, treatment, plot_id, leaf_area_cm2)
...
LT_analysis_CWM_site <- lmer(log(leaf_thickness_mm) ~ treatment*site + (1|plot_id),
                        data = traits_CWM_wide)
anova(LT_analysis_CWM_site)
summary(LT_analysis_CWM_site)

# Create newdate for CWM SLA
newdata_CWM_LT <- traits_CWM_wide %>% ungroup() %>% select(site, treatment, plot_id, leaf_thickness_mm)
...
LDMC_analysis_CWM_site <- lmer(log(ldmc) ~ treatment*site + (1|plot_id),
                        data = traits_CWM_wide)
anova(LDMC_analysis_CWM_site)
summary(LDMC_analysis_CWM_site)

# Create newdate for CWM SLA
newdata_CWM_LDMC <- traits_CWM_wide %>% ungroup() %>% select(site, treatment, plot_id, ldmc)
