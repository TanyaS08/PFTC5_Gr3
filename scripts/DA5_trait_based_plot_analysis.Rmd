...
for (i in 1:length(analysis_site_bs_raw)) {
  ...
  # Use 'predict' on models
  analysis_site_bs_predict[[i]]$predicted_value <- 
    predict(analysis_site_bs_raw[[i]], newdata = analysis_site_bs_predict[[i]], re.form = NULL, allow.new.levels = TRUE) 
  
}

do.call(rbind.data.frame, analysis_site_bs_predict[1:7]) %>% 
  as_tibble() %>%
  ggplot(aes(x = treatment, y = predicted_value, fill = site)) + 
  geom_violin(position=position_dodge(0.5), alpha = 0.3) + 
  #geom_boxplot(position=position_dodge(0.5), na.rm = TRUE, width = 0.2) +
  scale_fill_manual(values = c("ACJ" = "#F50054", 
                               "TRE" = "#DB6300",  
                               "QUE" = "#FFFA01"),
                    name = "Sites") +
  facet_wrap(vars(trait),
