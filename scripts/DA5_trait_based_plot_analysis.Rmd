...
SLA_analysis_site <- lmer(log(sla_cm2_g) ~ treatment*site + (1|plot_id) + (1|elevation)  + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(SLA_analysis_site)
summary(SLA_analysis_site)

# create new dataset with only relevant factors from the model
newdata_SLA <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, elevation,taxon, functional_group, season, sla_cm2_g)
...
# Leaf Area
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
LA_analysis_site <- lmer(log(leaf_area_cm2) ~ treatment*site + (1|plot_id) + (1|elevation)  +(1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(LA_analysis_site)
summary(LA_analysis_site)

# create new dataset with only relevant factors from the model
newdata_LA <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, elevation,taxon, functional_group, season, leaf_area_cm2)
...
# Plant Height
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
PH_analysis_site <- lmer(log(plant_height_cm) ~ treatment*site + (1|plot_id) +(1|elevation)  + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(PH_analysis_site)
summary(PH_analysis_site)

# create new dataset with only relevant factors from the model
newdata_PH <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, elevation,taxon, functional_group, season, plant_height_cm)
...
# Leaf Thickness
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
LT_analysis_site <- lmer(log(leaf_thickness_mm) ~ treatment*site + (1|plot_id) +(1|elevation)  + (1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(LT_analysis_site)
summary(LT_analysis_site)

# create new dataset with only relevant factors from the model
newdata_LT <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id, elevation,taxon, functional_group, season, leaf_thickness_mm)
...
# LDMC
# Run model without random effects causing issues (familY nested over taxon) and (site nested over plot_id)
LDMC_analysis_site <- lmer(log(ldmc) ~ treatment*site + (1|plot_id) + (1|elevation)  +(1|taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(LDMC_analysis_site)
summary(LDMC_analysis_site)

# create new dataset with only relevant factors from the model
newdata_LDMC <- traits_wide %>% ungroup() %>% select(site, treatment, plot_id,elevation, elevation, taxon, functional_group, season, ldmc)
