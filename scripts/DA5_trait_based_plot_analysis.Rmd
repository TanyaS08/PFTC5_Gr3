...
# Many issues with convergence (not resolved until we use CWM)
# SLA
SLA_analysis <- lmer(log(sla_cm2_g) ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon) + (1|functional_group) + (1|season), data = traits_wide)
anova(SLA_analysis)
summary(SLA_analysis)

# Leaf Area
LA_analysis <- lmer(log(leaf_area_cm2) ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon)+ (1|functional_group) + (1|season), data = traits_wide)
anova(LA_analysis)
summary(LA_analysis)

# Plant Height
PlantHeight_analysis <- lmer(log(plant_height_cm) ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon)+ (1|functional_group) + (1|season), data = traits_wide)
anova(PlantHeight_analysis)
summary(PlantHeight_analysis)

# Leaf thickness
LeafThickness_analysis <- lmer(log(leaf_thickness_mm) ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon)+ (1|functional_group) + (1|season), data = traits_wide)
anova(LeafThickness_analysis)
summary(LeafThickness_analysis)

# LDMC
LDMC_analysis <- lmer(ldmc ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon)+ (1|functional_group) + (1|season), data = traits_wide)
anova(LDMC_analysis)
...
# SLA
SLA_analysis_dry <- lmer(log(sla_cm2_g) ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon) + (1|functional_group), data = traits_wide_dry)
anova(SLA_analysis_dry)
summary(SLA_analysis_dry)

# Leaf Area
LA_analysis_dry <- lmer(log(leaf_area_cm2) ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon) + (1|functional_group), data = traits_wide_dry)
anova(LA_analysis_dry)
summary(LA_analysis_dry)

# Plant Height
PlantHeight_analysis_dry <- lmer(log(plant_height_cm) ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon) + (1|functional_group), data = traits_wide_dry)
anova(PlantHeight_analysis_dry)
summary(PlantHeight_analysis_dry)

# Leaf thickness
LeafThickness_analysis_dry <- lmer(log(leaf_thickness_mm) ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon) + (1|functional_group), data = traits_wide_dry)
anova(LeafThickness_analysis_dry)
summary(LeafThickness_analysis_dry)

# LDMC
LDMC_analysis_dry <- lmer(ldmc ~ treatment*elevation + (1|site/plot_id) + (1|family/taxon) + (1|functional_group), data = traits_wide_dry)
anova(LDMC_analysis_dry)
summmary(LDMC_analysis_dry)
```
...
table(traits_wide$site, traits_wide$plot_id, traits_wide$elevation)
# NB. Site and elevation are not synonymous - multiple elevations per site (plots)
# So perhaps match by plot_id and elevation, rather than site
# However, it seems multiple plots are represented at 3400masl for example..

traits_CWM <- traits_CWM %>%
    mutate(
         elevation_ave = case_when(site == "ACJ" ~ "3468",
                           site == "TRE" ~ "3715",
                           site == "QUE" ~ "3888"))
         
# Pivot wider
traits_CWM_wide <- traits_CWM %>% pivot_wider(names_from = c(trait),
                                      values_from = c(estimate))


### Linear mixed effects models with average elevation for now
# SLA
SLA_analysis_CWM <- lmer(log(sla_cm2_g) ~ treatment*elevation + (1|site/plot_id),
                         data = traits_CWM_wide)
anova(SLA_analysis_CWM)
summary(SLA_analysis_CWM)

# Leaf Area
LA_analysis_CWM <- lmer(log(leaf_area_cm2) ~ treatment*elevation + (1|site/plot_id),
                        data = traits_CWM)
anova(LA_analysis_CWM)
summary(LA_analysis_CWM)

# Plant Height
PlantHeight_analysis_CWM <- lmer(log(plant_height_cm) ~ treatment*elevation + (1|site/plot_id), data = traits_CWM)
anova(PlantHeight_analysis_CWM)
summary(PlantHeight_analysis_CWM)

# Leaf thickness
LeafThickness_analysis_CWM <- lmer(log(leaf_thickness_mm) ~ treatment*elevation + (1|site/plot_id), data = traits_CWM)
anova(LeafThickness_analysis_CWM)
summary(LeafThickness_analysis_CWM)

# LDMC
LDMC_analysis_CWM <- lmer(ldmc ~ treatment*elevation + (1|site/plot_id),
                          data = traits_CWM)
